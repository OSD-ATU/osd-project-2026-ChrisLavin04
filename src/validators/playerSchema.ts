...
export const createPlayerSchema = z.object({
  ...
  team_id: z.string()
    ...
    .optional(),
  goals: z.number().int().min(0, { message: "Goals must be a non-negative integer" }).optional(),
  assists: z.number().int().min(0, { message: "Assists must be a non-negative integer" }).optional(),
  matchesPlayed: z.number().int().min(0, { message: "Matches played must be a non-negative integer" }).optional()
});
